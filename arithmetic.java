...
public class JavaToAssembly {
	...
	private void outputToAsm(String fileName){
		...
		try
		{
			outputStream = new PrintWriter(new FileOutputStream("test.asm"));
		...
		for(int j = 0; j<code.size();j++){
			...
			if((line.contains("++") || line.contains("--")) && !line.contains("for")){
				...
				if(line.contains("+")){
					var = line.substring(0, line.indexOf("="));
					var = var.replace(" ", "");
					var2 = line.substring(line.indexOf("=")+1, line.indexOf("+"));
					var2 = var2.replace(" ", "");
					var3 = line.substring(line.indexOf("+")+1, line.indexOf(";"));
					var3 = var3.replace(" ", "");
				}
				else{
					var = line.substring(0, line.indexOf("="));
				...
				if(var.equalsIgnoreCase(var2))
					asm.append("mov al, " + var3 + "\n");
				else if(var.equalsIgnoreCase(var3))
					asm.append("mov al, " + var2 + "\n");
				else{
					asm.append("mov al, " + var2 + "\n");
					if(operation==0)
						asm.append("add al, " + var3 + "\n");
					else
						asm.append("add al, " + var3 + "\n");
				}
				if(operation==0)
					asm.append("add " + var + ", al" + "\n");
				else
					asm.append("sub " + var + ", al" + "\n");
